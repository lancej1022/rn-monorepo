...
export function LoginForm({ style, ...props }: React.ComponentPropsWithoutRef<typeof View>) {
  ...
  const handleForgotPassword = () => {
    // Navigate to forgot password screen or open URL
    // For demo purposes, using Linking
    Linking.openURL('#');
  };

  const handleSignUp = () => {
    // Navigate to sign up screen or open URL
    Linking.openURL('#');
  };
  ...
  return (
    <ScrollView className={cn('flex-1', style)} keyboardShouldPersistTaps='handled' {...props}>
      <View className='flex flex-col gap-6'>
        <View className='flex flex-col items-center gap-2'>
          <Text className='text-2xl font-bold text-center'>Login to your account</Text>
          <Text className='text-balance text-sm text-muted-foreground text-center'>
        ...
        <View className='gap-6'>
          ...
          <View className='gap-2'>
            <View className='flex-row items-center justify-between'>
              ...
              <Pressable onPress={handleForgotPassword}>
                <Text className='text-sm underline-offset-4'>Forgot your password?</Text>
              </Pressable>
            </View>
          ...
          <View className='relative items-center justify-center py-2'>
            <View className='absolute h-px bg-border w-full' />
            <View className='bg-background px-2 z-10'>
              <Text className='text-muted-foreground text-sm'>Or continue with</Text>
            </View>
          </View>

          <Button variant='outline' className='w-full' onPress={handleGitHubLogin}>
